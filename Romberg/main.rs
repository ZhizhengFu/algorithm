...
fn romberg_integration(a: f64, b: f64, f: fn(f64) -> f64) -> f64 {
    let mut t = [[0.0; MAX_ITERATIONS]; 2];
    t[0][0] = trapezoidal(a, b, 1, f);
    println!("T[0][0] = {:.8}", t[0][0]);
    for j in 1..MAX_ITERATIONS {
        t[1][0] = trapezoidal(a, b, 1 << j, f);
        print!("T[{}][0] = {:.8} ", j, t[1][0]);
        for k in 1..=j {
...
fn main() {
    ...
    for (i, &start_end) in range.iter().enumerate() {
        ...
        println!("\n龙贝格积分法：");
        romberg_integration(start, end, func_ptr[i]);
    }
}
